...
const postgresBinaries = ['initdb', 'pg_ctl', 'psql', 'createdb'] as const;
  ...
  () => {
    it('rolls back failures and creates leak-free least-privilege roles', async () => {
      ...
      try {
        ...
        const runBootstrap = (): Promise<CommandResult> =>
        ...
        query(port, 'create role portfolio_backup');
        ...
        expect(failedBootstrap.status).not.toBe(0);
        assertTemporarySqlRemoved(sqlDirectory);
        expect(
          query(
            port,
            `select
                 count(*) filter (where rolname = 'portfolio_migrator'),
                 count(*) filter (where rolname = 'portfolio_app'),
                 count(*) filter (where rolname = 'portfolio_backup'),
        ...
        ).toBe('0|0|1|postgres|t|t');

        query(port, 'drop role portfolio_backup');
